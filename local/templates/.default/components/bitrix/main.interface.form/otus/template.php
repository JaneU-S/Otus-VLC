...
foreach($tab["fields"] as $field):
	...
	if(isset($field['property_code']) && $field['property_code'] !== 'LEGAL_ENTITY_ABROAD'){
		
		$className[] = 'le-row';

		if($arResult["XML_ID"] == 'no'){
			$className[] = 'hidden';
		}
	}
...
<select name="<?=$field["id"]?>"<?=$params?> <?if(isset($field["property_code"])):?>id="<?=$field["property_code"]?>"<?endif?>>
<?
...
?>
<?if(isset($field["property_value"])):?>
	<option value="<?=htmlspecialcharsbx($k)?>"<?=(in_array($k, $val)? ' selected':'')?> 
	<?if($field["le_abroad_id"] == $k):?>data-le-abroad="yes"<?else:?>data-le-abroad="no"<?endif?>>
...
var show = function(){
	var rows = document.querySelectorAll('.le-row');
	rows.forEach((item) => {
...
var selected = document.getElementById('LEGAL_ENTITY_ABROAD');
console.log(selected);
selected.addEventListener("change", (event) => {	
	let le_abroad = event.target.selectedOptions[0].getAttribute('data-le-abroad');
	if(le_abroad == 'yes'){
		show();
	}else{
			hide();
